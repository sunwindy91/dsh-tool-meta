...
## 十二、第三层留痕：告警渠道本身也会失败（2026-09-22）

### 一次真实的"没告成警"

配套的自动化里有一道降级检测：`gh` 大量失败或候选为 0 → 标记降级并**主动告警**。
2026-09-21 它真的生效了：

```
[heartbeat] ⚠️ 降级 · 降级运行：gh 失败 28 次；候选 0 条
! 飞书推送失败: URLError: <urlopen error [Errno 11001] getaddrinfo failed>
```

**降级检测是对的，但告警没送到。**
原因：故障本身就是"没网"（DNS 都解析不了），而告警渠道（飞书）也要网。

> **监控的经典盲区：报警器依赖了它要监控的那个东西。**

### 因此定下三层留痕

| 层 | 留什么 | 失败时怎么办 |
|---|---|---|
| **一、动作留痕** | 不可逆动作必须留下声明（谁、几点、删什么、依据什么备份） | 闸门直接拒绝 |
| **二、结果留痕** | 每次运行写下 `exit=`；降级运行必须显式标记 | 日志可追 |
| **三、渠道留痕** | **告警送不出去时，先落本地队列**；下次联网自动补送 | `alerts-pending.jsonl`（store-and-forward） |

**一句话**：**任何"用来发现失败"的机制，本身也必须有一个不依赖同一个前提的兜底。**

### 附：一条测试纪律（我自己踩出来的）

测试"对外动作"（发消息、推送、发布）之前，**先确认它的爆炸半径**。
真实事故：测试告警时把假 webhook 写在**配置层**，而取值优先级是
**环境变量 > 独立文件 > 配置** → 脚本读了真地址，**一条"冒烟测试"消息真的发进了群，且无法撤回**。

**规矩**：覆盖凭据必须用**环境变量**（优先级最高）；测试前先问一句"如果它真的发出去，会发到哪里"。
