...
export function detectDanger(exec) {
  ...
  const isCode = CODE_DELETE_SIGNALS.some((re) => re.test(text));
  const isCli = CLI_DELETE_SIGNALS.some((re) => re.test(text));
  if (!isCode && !isCli) return null;

  const kind = isCode ? "code" : "explicit";
  const targets = kind === "explicit" ? extractTargets(argsText) : [];
  const wildcard = kind === "explicit" && targets.some((t) => WILDCARD.test(t));
  const forceRecurse = FORCE_RECURSE.some((re) => re.test(text));
  return { kind, targets, wildcard, forceRecurse, raw: text.slice(0, 300) };
}
